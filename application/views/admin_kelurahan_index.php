...
<body>
  ...
  <button id="menuToggle" onclick="toggleSidebar()" style="display:none;background:none;border:none;padding:4px 8px 4px 0;cursor:pointer;" aria-label="Buka menu">
    <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="#475569"><path stroke-width="1.8" d="M4 6h16M4 12h16M4 18h16"/></svg>
  </button>
  <div>
    <span class="topbar-title">Manajemen Kelurahan</span>
    <span class="topbar-sub">— Kecamatan & Kelurahan Kota Tomohon</span>
  </div>
...
<!-- CONTENT -->
					...
					<ul class="nav nav-tabs mb-4 px-1" id="wilayahTab" role="tablist" style="border-bottom: 1px solid var(--border); gap: 4px;">
						...
						<li class="nav-item" role="presentation" style="margin-left: auto;">
							<button class="nav-link" id="kelola-kec-btn" data-bs-toggle="tab" data-bs-target="#tab-kelola-kec" type="button" role="tab" style="color:var(--danger)">
								<svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="me-1">
										...
										<table class="table-custom">
											...
											<tbody>
												...
												?>
														<tr>
															<td>
																<div class="item-title"><?= htmlspecialchars($kel['nama_kelurahan']) ?></div>
															</td>
															<td>
												...
												<?php if (!$found): ?>
													<tr>
														<td colspan="3" class="text-center py-4 text-muted" style="font-size:13px">Belum ada data kelurahan di kecamatan ini.</td>
													</tr>
												<?php endif; ?>
											</tbody>
										</table>
								...
								<div class="table-responsive flex-grow-1">
									<table class="table-custom">
										...
										<tbody>
											<?php if (empty($kecamatan)): ?>
												<tr>
													<td colspan="2" class="text-center py-4 text-muted" style="font-size:13px">Belum ada data kecamatan.</td>
												</tr>
											<?php else: ?>
												<?php foreach ($kecamatan as $k): ?>
													<tr>
														<td>
															<div class="item-title"><?= htmlspecialchars($k['nama_kecamatan']) ?></div>
														</td>
														<td class="text-end">
	...
	<script>
		const DEFAULT_LAT = 1.3165,
			DEFAULT_LNG = 124.8300,
			DEFAULT_ZOOM = 13;
