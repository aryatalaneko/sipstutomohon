			...
			<div class="content">
					...
					<div style="padding: 28px; display:flex; justify-content:space-between; align-items:center;">
						<div>
							...
							<div style="display:flex; gap:20px; font-size:13px; color:rgba(255,255,255,0.7); font-weight:500;">
								<span style="display:flex; align-items:center; gap:6px;"><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
										<path stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
									</svg> Lahir: <?= date('d M Y', strtotime($b['tgl_lahir'])) ?></span>
								<?php
								...
								?>
								<span style="display:flex; align-items:center; gap:6px;"><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
										<path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
									</svg> Umur: <?= $age_str ?></span>
								<span style="display:flex; align-items:center; gap:6px;"><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
									...
									</svg> Kel. <?= htmlspecialchars($b['nama_kelurahan']) ?></span>
							</div>
						</div>
						<div>
				...
				<div style="display:flex; align-items:center; gap:12px; margin-bottom:16px;">
					<div style="width:32px; height:32px; background:var(--blue-50); color:var(--blue-600); border-radius:8px; display:flex; align-items:center; justify-content:center;">
						<svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							<path stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012-2h-2a2 2 0 01-2-2z" />
						</svg>
					</div>
						...
						<table class="table-custom">
							...
							<tbody>
								<?php if (empty($pengukuran)): ?>
									<tr>
										<td colspan="7" class="text-center py-5 text-muted" style="font-size:13px;">Belum ada rekam jejak pengukuran untuk balita ini.</td>
									</tr>
								<?php else: ?>
									<?php foreach ($pengukuran as $p): ?>
										<tr>
											<td>
												<div style="font-weight:600; color:var(--text-primary);"><?= date('d F Y', strtotime($p['tgl_pengukuran'])) ?></div>
											</td>
											<td>
												<div class="item-title"><?= $p['berat_badan'] ?></div>
											</td>
											<td>
												<div class="item-title"><?= $p['tinggi_badan'] ?></div>
											</td>
											<td>
											...
											<td>
												...
												$color = 'var(--success)';
												$bg = '#ecfdf5';
												if (strpos($s, 'stunting') !== false && strpos($s, 'sangat') === false) {
													$color = '#b45309';
													$bg = '#fffbeb';
												} else if (strpos($s, 'sangat') !== false || strpos($s, 'severe') !== false) {
													$color = '#b91c1c';
													$bg = '#fef2f2';
												}
												?>
